...
-- IMPORTANT: SCI's existing Prisma IDs are TEXT/CUID values, not PostgreSQL UUIDs.
...
BEGIN
  ...
  UPDATE inspection_templates SET "isDefault" = false WHERE "organizationId" = org_id;
  SELECT id INTO tpl_id FROM inspection_templates
    WHERE "organizationId" = org_id AND code = 'STANDARD_PROPERTY' AND version = 2 LIMIT 1;
  IF tpl_id IS NULL THEN
    INSERT INTO inspection_templates (id, "organizationId", code, name, description, version, "isDefault", status, "createdAt", "updatedAt")
    VALUES (gen_random_uuid()::text, org_id, 'STANDARD_PROPERTY', 'Standard property inspection',
      'Client workflow with conditional vacant/improved property data, valuation and evidence.', 2, true, 'ACTIVE', now(), now())
    RETURNING id INTO tpl_id;
  ELSE
    UPDATE inspection_templates SET "isDefault"=true, status='ACTIVE', "updatedAt"=now() WHERE id=tpl_id;
  END IF;
  ...
  FOR s IN SELECT * FROM jsonb_array_elements('[
    ...
    {"code":"MAIN_BUILDING","name":"Main building","order":3,"fields":[
      ...
      {"code":"INDUSTRIAL_UNITS","label":"Industrial units / bays","type":"NUMBER","required":false,"validation":{"visibleWhen":{"all":[{"field":"PROPERTY_STATUS","equals":"IMPROVED"},{"field":"BUILDING_USE","equals":"INDUSTRIAL"}]}}},
    ...
    INSERT INTO template_sections (id, "templateId", code, name, "isAssessment", "sortOrder")
      VALUES (gen_random_uuid()::text, tpl_id, s->>'code', s->>'name', false, (s->>'order')::int)
      ON CONFLICT ("templateId", code) DO UPDATE SET name=EXCLUDED.name, "sortOrder"=EXCLUDED."sortOrder"
    ...
    FOR f IN SELECT * FROM jsonb_array_elements(COALESCE(s->'fields','[]'::jsonb)) LOOP
      ...
      VALUES (gen_random_uuid()::text, sec_id, f->>'code', f->>'label', (f->>'type')::"FieldType", COALESCE((f->>'required')::boolean,false), n,
  ...
  DELETE FROM template_photo_rules WHERE "templateId" = tpl_id;
  INSERT INTO template_photo_rules (id, "templateId", category, "minCount", required, description) VALUES
    (gen_random_uuid()::text,tpl_id,'FRONT_VIEW',1,true,'Property frontage / access'),
    (gen_random_uuid()::text,tpl_id,'SURROUNDINGS',1,true,'Plot or property surroundings'),
    (gen_random_uuid()::text,tpl_id,'ROAD_ACCESS',1,true,'Road access'),
    (gen_random_uuid()::text,tpl_id,'INTERIOR',1,false,'Interior / main building'),
    (gen_random_uuid()::text,tpl_id,'ROOF',1,false,'Roof'),
    (gen_random_uuid()::text,tpl_id,'DOCUMENT',1,false,'Land title / UPI / lease document'),
    (gen_random_uuid()::text,tpl_id,'OTHER',1,false,'Other evidence');
END $$;
