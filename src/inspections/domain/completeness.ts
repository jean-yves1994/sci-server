...
function rawValue(value: SubmittedValue | undefined): unknown {
  ...
  if (value.valueNumber !== undefined && value.valueNumber !== null) return value.valueNumber;
  if (value.valueBool !== undefined && value.valueBool !== null) return value.valueBool;
...
function matchesRule(rule: VisibilityRule | undefined, valuesByCode: Map<string, unknown>): boolean {
  ...
  if (rule.any && !rule.any.some((r) => matchesRule(r, valuesByCode))) return false;
  if (rule.field) {
    const actual = valuesByCode.get(rule.field);
    if (rule.equals !== undefined && !sameValue(actual, rule.equals)) return false;
    if (rule.min !== undefined && (typeof actual !== 'number' || actual < rule.min)) return false;
    if (rule.max !== undefined && (typeof actual !== 'number' || actual > rule.max)) return false;
  }
  return true;
}
...
function validateFieldValue(field: FieldDefinition, value: SubmittedValue, sectionCode: string): CompletenessIssue[] {
  ...
  if (typeof value.valueNumber === 'number') {
    if (rules.min !== undefined && value.valueNumber < rules.min) issues.push({ code: 'VALUE_BELOW_MIN', sectionCode, fieldCode: field.code, message: `${field.label} must be at least ${rules.min}.`, blocking: true });
    if (rules.max !== undefined && value.valueNumber > rules.max) issues.push({ code: 'VALUE_ABOVE_MAX', sectionCode, fieldCode: field.code, message: `${field.label} must be no more than ${rules.max}.`, blocking: true });
  }
...
export function evaluateCompleteness(input: CompletenessInput): CompletenessResult {
  ...
  for (const assessment of input.assessments) {
    ...
    else issues.push({ code: 'ASSESSMENT_NOT_RATED', sectionCode: assessment.categoryCode, message: `${assessment.categoryName} has not been rated.`, blocking: true });
  }
...
export function describePhotoCategory(category: PhotoCategory | string): string {
  const labels: Record<string, string> = { FRONT_VIEW: 'Front view', REAR_VIEW: 'Rear view', LEFT_SIDE: 'Left side', RIGHT_SIDE: 'Right side', INTERIOR: 'Interior', ROOF: 'Roof', FOUNDATION: 'Foundation', ROAD_ACCESS: 'Road access', UTILITIES: 'Utilities', SURROUNDINGS: 'Surroundings', DOCUMENT: 'Document', OTHER: 'Other' };
