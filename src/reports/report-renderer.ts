...
export class ReportRenderer {
  ...
  private header(doc: PDFKit.PDFDocument, data: ReportData) {
    ...
    doc.fillColor(C.ink); doc.y = 118;
  }

  private summary(doc: PDFKit.PDFDocument, data: ReportData) {
    const top = doc.y;
    doc.rect(MARGIN, top, CONTENT_WIDTH, 74).fill(C.panel); doc.fillColor(C.ink);
    ...
    [['Inspection', data.inspection.inspectionNumber], ['Loan reference', data.inspection.loanReference], ['Branch', data.inspection.branch], ['Client', data.inspection.clientName ?? '—'], ['Inspector', data.people.inspector ?? '—'], ['Reviewer', data.people.reviewer ?? '—']].forEach(([l, v], i) => {
      const x = MARGIN + 12 + (i % 3) * col; const y = top + 12 + Math.floor(i / 3) * 30;
  ...
  private property(doc: PDFKit.PDFDocument, data: ReportData) {
    ...
    this.section(doc, 'Land registration'); this.kv(doc, [['UPI', data.property.titleNumber?.trim() || 'Not recorded']]);
  }
  ...
  private fields(doc: PDFKit.PDFDocument, data: ReportData) {
    const visible = data.fieldValues.filter(e => !this.isLegacyPlotNumber(e)); if (!visible.length) return;
    this.section(doc, 'Recorded information'); let current = '';
    for (const e of visible) {
      this.ensure(doc, 34);
      if (e.section !== current) { current = e.section; doc.moveDown(0.3).font('Helvetica-Bold').fontSize(8.5).fillColor(C.brand).text(current.toUpperCase(), MARGIN, doc.y); doc.moveDown(0.2); }
  ...
  private assessments(doc: PDFKit.PDFDocument, data: ReportData) {
    this.section(doc, 'Condition assessment'); if (!data.assessments.length) { this.note(doc, 'No assessment categories were recorded.'); return; }
    const x1 = MARGIN + 6, x2 = MARGIN + 210, x3 = MARGIN + 285; this.ensure(doc, 26);
    doc.rect(MARGIN, doc.y, CONTENT_WIDTH, 18).fill(C.panel); doc.fillColor(C.muted).font('Helvetica-Bold').fontSize(7.5);
    doc.text('CATEGORY', x1, doc.y + 5.5); doc.text('RATING', x2, doc.y + 5.5); doc.text('CONDITION', x3, doc.y + 5.5); doc.y += 22;
    for (const a of data.assessments) {
      this.ensure(doc, 30); const y = doc.y;
      doc.font('Helvetica-Bold').fontSize(9).fillColor(C.ink).text(a.categoryName, x1, y, { width: 195 });
      doc.font('Helvetica').fontSize(9).text(a.rating !== null ? `${a.rating} / 5` : '—', x2, y, { width: 70 }); doc.text(this.human(a.condition), x3, y, { width: 120 });
      ...
      doc.moveTo(MARGIN, bottom + 5).lineTo(MARGIN + CONTENT_WIDTH, bottom + 5).strokeColor(C.rule).lineWidth(0.4).stroke(); doc.y = bottom + 11;
    }
  }

  private valuation(doc: PDFKit.PDFDocument, data: ReportData) {
    this.section(doc, 'Valuation'); if (!data.valuation) { this.note(doc, 'No valuation was recorded.'); return; }
    const v = data.valuation; const rows: Array<[string, string]> = [];
  ...
  private async photos(doc: PDFKit.PDFDocument, data: ReportData) {
    if (!data.photos.length) return; if (!this.fresh(doc)) doc.addPage(); this.section(doc, 'Photographic evidence');
    const gap = 14, cell = (CONTENT_WIDTH - gap) / 2, imageH = 138, rowH = imageH + 40, bottom = doc.page.height - MARGIN; let col = 0, rowTop = doc.y;
    for (const p of data.photos) {
      if (col === 0 && rowTop + rowH > bottom) { doc.addPage(); rowTop = doc.y; }
      const x = MARGIN + col * (cell + gap);
      try { const b = await this.storage.get(p.storageKey); doc.image(b, x, rowTop, { fit: [cell, imageH], align: 'center', valign: 'center' }); }
      catch (e) { this.logger.warn(`Photo unavailable: ${String(e)}`); doc.rect(x, rowTop, cell, imageH).fill(C.panel); doc.fillColor(C.muted).fontSize(8).text('Image unavailable', x, rowTop + imageH / 2 - 4, { width: cell, align: 'center' }); }
      ...
      col++; if (col >= 2) { col = 0; rowTop += rowH; doc.y = rowTop; }
    }
  ...
  private pageNumbers(doc: PDFKit.PDFDocument, data: ReportData) {
    ...
    for (let i = range.start; i < range.start + range.count; i++) { doc.switchToPage(i); doc.font('Helvetica').fontSize(7).fillColor(C.muted).text(`Report ${data.reportNumber} · Page ${i + 1} of ${range.count}`, MARGIN, doc.page.height - 28, { width: CONTENT_WIDTH, align: 'center' }); }
  }
  private section(doc: PDFKit.PDFDocument, title: string) { this.ensure(doc, 42); doc.moveDown(0.6); doc.font('Helvetica-Bold').fontSize(12).fillColor(C.ink).text(title, MARGIN, doc.y); doc.moveTo(MARGIN, doc.y + 4).lineTo(MARGIN + CONTENT_WIDTH, doc.y + 4).strokeColor(C.rule).lineWidth(0.6).stroke(); doc.moveDown(0.45); }
  private kv(doc: PDFKit.PDFDocument, rows: Array<[string, string]>) { for (const [l, v] of rows) { this.ensure(doc, 22); const y = doc.y; doc.font('Helvetica').fontSize(7.5).fillColor(C.muted).text(l, MARGIN + 6, y, { width: 150 }); doc.font('Helvetica').fontSize(9).fillColor(C.ink).text(v, MARGIN + 160, y, { width: CONTENT_WIDTH - 166, ellipsis: true }); doc.moveDown(0.45); } }
  private note(doc: PDFKit.PDFDocument, t: string) { this.ensure(doc, 28); doc.font('Helvetica-Oblique').fontSize(8.5).fillColor(C.muted).text(t, MARGIN + 6, doc.y); doc.moveDown(0.5); }
