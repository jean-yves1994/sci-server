...
type BufferedPage = { content?: { buffer?: Array<Uint8Array> }; dictionary?: { data?: Record<string, unknown> } };
type PdfInternals = PDFKit.PDFDocument & {
  _pageBuffer?: BufferedPage[];
  _pageBufferStart?: number;
...
export class ReportRenderer {
  ...
  async render(data: ReportData): Promise<Buffer> {
    ...
    this.removeEmptyBufferedPages(doc);
  ...
  private async photos(doc: PDFKit.PDFDocument, data: ReportData) {
    ...
    for (const p of data.photos) {
      if (col === 0) {
        // Only create a page when the next photo row cannot fit. This avoids
        // speculative pages at the end of the report.
        if (rowTop + rowH > bottom) { doc.addPage(); rowTop = doc.y; }
      }
  ...
  private removeEmptyBufferedPages(doc: PDFKit.PDFDocument) {
    const internal = doc as PdfInternals;
    const pages = internal._pageBuffer;
    if (!pages || pages.length <= 1) return;

    const kids = internal._root?.data?.Pages?.data?.Kids;
    const pagesData = internal._root?.data?.Pages?.data;

    // PDFKit keeps a page content stream in each buffered page. A page that
    // contains only the initial graphics-state setup/teardown is empty for
    // report purposes. Do not use a regex over the entire stream: PDF
    // operators are short tokens and can be present in unrelated binary/text
    // data, which made the previous cleanup unreliable.
    const isEmpty = (page: BufferedPage) => {
      const chunks = page.content?.buffer ?? [];
      if (!chunks.length) return true;
      const stream = Buffer.concat(chunks.map(chunk => Buffer.from(chunk))).toString('latin1');
      const normalized = stream.replace(/\s+/g, ' ').trim();
      return normalized === '' || normalized === 'q Q' || normalized === 'q Q q Q';
    };

    for (let i = pages.length - 1; i >= 0; i--) {
      if (!isEmpty(pages[i])) break;
      pages.splice(i, 1);
      if (kids && i < kids.length) kids.splice(i, 1);
      if (pagesData && typeof pagesData.Count === 'number') pagesData.Count = Math.max(1, pagesData.Count - 1);
    }
  }
  ...
  private pageNumbers(doc: PDFKit.PDFDocument) {
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      doc.switchToPage(i);
      doc.font('Helvetica').fontSize(7).fillColor(C.muted).text(`Page ${i + 1} of ${range.count}`, MARGIN, doc.page.height - 28, { width: CONTENT_WIDTH, align: 'center' });
    }
  }
