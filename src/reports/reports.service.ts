...
export class ReportsService {
  ...
  async generate(user: TenantContext, inspectionId: string, meta: RequestMetadata) {
    ...
    const existingCount = await this.prisma.report.count({ where: { inspectionId } }); const version = existingCount + 1; const reportNumber = this.buildReportNumber(inspection.inspectionNumber);
    const pdf = await this.renderer.render(this.toReportData(inspection, reportNumber, version, user));
    const storageKey = `reports/${inspection.organizationId}/${reportNumber}-v${version}.pdf`; const stored = await this.storage.put(storageKey, pdf, 'application/pdf');
    return this.prisma.runInTransaction(async (tx) => {
      ...
      await this.audit.record({ organizationId: inspection.organizationId, userId: user.userId, action: 'REPORT_GENERATED', entityType: 'Report', entityId: created.id, metadata: { reportNumber, version, inspectionNumber: inspection.inspectionNumber, checksum: stored.checksumSha256 }, meta }, tx);
      return created;
    });
  }
  async list(user: TenantContext, query: { page: number; pageSize: number; search?: string }): Promise<PaginatedResult<unknown>> {
    const scope = buildTenantScope(user); const where = { organizationId: scope.organizationId, ...(scope.branchId ? { inspection: { branchId: scope.branchId } } : {}), ...(query.search ? { OR: [{ reportNumber: { contains: query.search, mode: 'insensitive' as const } }, { inspection: { inspectionNumber: { contains: query.search, mode: 'insensitive' as const } } }, { inspection: { loanReference: { contains: query.search, mode: 'insensitive' as const } } }] } : {}) };
  ...
  private toReportData(inspection: NonNullable<Awaited<ReturnType<ReportsService['loadForReport']>>>, reportNumber: string, version: number, user: TenantContext): ReportData {
    ...
    const fieldValues = inspection.values.map((value) => ({ section: value.field.section.name, sortOrder: value.field.sortOrder, label: value.field.label, value: this.stringifyValue(value) })).filter((entry) => entry.value !== '').sort((a, b) => a.section.localeCompare(b.section) || a.sortOrder - b.sortOrder);
    const propertyAddress = [inspection.property.villageStreet, inspection.property.cell, inspection.property.sector, inspection.property.district, inspection.property.province].filter((value): value is string => Boolean(value?.trim())).join(', ');
    return {
      ...
      reportNumber, version, generatedAt: new Date(), generatedBy: user.userId,
      ...
      property: { reference: inspection.property.reference, propertyType: inspection.property.propertyType, addressLine: propertyAddress || inspection.property.addressLine || '—', titleNumber: inspection.property.titleNumber, division: inspection.property.division?.name ?? null },
      ...
      valuation: inspection.valuation ? { currency: inspection.valuation.currency, marketValue: inspection.valuation.marketValue === null ? null : Number(inspection.valuation.marketValue), forcedSaleValue: inspection.valuation.forcedSaleValue === null ? null : Number(inspection.valuation.forcedSaleValue), replacementCost: inspection.valuation.replacementCost === null ? null : Number(inspection.valuation.replacementCost), rentalEstimate: inspection.valuation.rentalEstimate === null ? null : Number(inspection.valuation.rentalEstimate), comments: inspection.valuation.comments } : null,
      fieldValues: fieldValues.map(({ section, label, value }) => ({ section, label, value })), photos: inspection.photos.map((p) => ({ category: p.category, storageKey: p.storageKey, caption: p.caption, capturedAt: p.capturedAt })),
      reviewerComments: inspection.comments.map((c) => ({ author: `${c.author.firstName} ${c.author.lastName}`, body: c.body, createdAt: c.createdAt, type: c.type })), timeline: inspection.statusEvents.map((e) => ({ toStatus: e.toStatus, actor: e.actor ? `${e.actor.firstName} ${e.actor.lastName}` : null, createdAt: e.createdAt, comment: e.comment })),
    };
  }
  private stringifyValue(value: { valueText: string | null; valueNumber: unknown; valueDate: Date | null; valueBool: boolean | null; valueJson: unknown }): string { if (value.valueText) return value.valueText; if (value.valueNumber !== null && value.valueNumber !== undefined) return String(value.valueNumber); if (value.valueDate) return new Date(value.valueDate).toLocaleDateString('en-GB'); if (value.valueBool !== null && value.valueBool !== undefined) return value.valueBool ? 'Yes' : 'No'; if (Array.isArray(value.valueJson)) return value.valueJson.join(', '); if (value.valueJson) return String(value.valueJson); return ''; }
}
